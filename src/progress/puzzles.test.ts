...
describe('puzzle completion persistence', () => {
  it('stores ids append-only under djl.v1.puzzles and pays only once', () => {
    ...
    expect(window.localStorage.getItem('djl.v1.puzzles')).toContain('alphabet-1-match')
  })

  it('survives corrupt and denied storage like the other progress stores', () => {
    window.localStorage.setItem('djl.v1.puzzles', '{bad')
