...
function renderMission(path = '/opdag/ord/mizu') {
  return render(
...
function finishGuideAndRecall() {
  ...
  fireEvent.click(screen.getByRole('button', { name: 'Vælg み, næste tegn' }))
  fireEvent.click(screen.getByRole('button', { name: 'Vælg ず, næste tegn' }))
  ...
  fireEvent.click(screen.getByRole('button', { name: 'Vælg み' }))
  fireEvent.click(screen.getByRole('button', { name: 'Vælg ず' }))
}
...
describe('ChildWordMission', () => {
  it('models, guides, retrieves, and permanently collects the word', () => {
    ...
    expect(document.querySelector('.child-word__saved')).toHaveTextContent('Nu er みず i din samling.')
    expect(getChildCollection()).toEqual(['mizu'])
  ...
  it('shows a local selected state and complete recovery after a wrong guided tile', () => {
    renderMission()
    fireEvent.click(screen.getByRole('button', { name: 'Byg ordet' }))
    const wrong = screen.getByRole('button', { name: 'Vælg ず' })
    ...
    expect(screen.getByRole('button', { name: 'Vælg み, næste tegn' })).toBeVisible()
  })
  ...
  it('returns an unknown mission to the workshop', () => {
    renderMission('/opdag/ord/ukendt')
