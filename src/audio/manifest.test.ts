...
describe('pronunciation audio manifest', () => {
  it('accepts only reviewed, local Japanese audio with explicit provenance', () => {
    ...
    for (const row of pronunciationAudio) {
      ...
      expect([form?.ja, form?.jaMarked], row.clipId).toContain(row.transcript)
    }
  })

  it('uses the app base path for public audio files', () => {
    expect(pronunciationAudioUrl(
      '/audio/example.mp3', '/Danish-Japanese-Lessons/app/',
    )).toBe('/Danish-Japanese-Lessons/app/audio/example.mp3')
  })
})
