...
Status: implementation complete; native Japanese review remains a release gate.
...
The app assumes no spoken, written, or reading knowledge of Japanese. It teaches before it tests,
recommends orientation → alphabet → name → vocabulary, and never locks a route. Every app-owned
Japanese letter, mark, word, sign, symbol, and phrase has Danish help, dansk lydskrift, and standard
...
- Letter sounds and displayed Japanese letter names are separate entries. UI phrases are complete
  phrase entries, not raw constants.
- Teaching diacritics belong in `jaMarked`; UI phrases remain undiacriticized.
- Deliberately taught Japanese numerals are symbols. Decorative and dynamic counters use Latin/Danish
  numerals.
...
- Learner names retain their Latin spelling and get letter-by-letter Japanese help. The app never
  fabricates whole-name IPA. Greetings and praise render a static catalog phrase and the name as
  separate segments.

## Shared rendering

- Full teaching card: Japanese → dansk lydskrift and IPA → Danish meaning.
...
- Personal-name companion: Japanese spelling + original name + letter-by-letter help.
...
- [x] Unique IDs, required fields, code points, ZWNJ, numeral, and `jaMarked` rules are tested.
- [x] A TypeScript source guard rejects uncatalogued Japanese literals in production render code.
- [x] `lang="ja"` production paths use approved renderers or the learner-input exception.
...
- [ ] Native Japanese review approves every new phrase, pronunciation, IPA value, sign explanation,
...
updates remain outside this plan. The Japanese landing page remains written for Japanese readers.
