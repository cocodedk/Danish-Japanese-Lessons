# Plan 005 — On-Screen Japanese Keyboard & Typing Exercises
...
The learner types Japanese on an in-app keyboard — no OS keyboard dependency — through typing
...
3. **Typing exercise** `src/pages/TypeWordScreen.tsx`: prompt = the Danish word + pronunciation
   line (never the Japanese answer); the learner types; live preview renders joined Naskh on the
   ruled sheet. Submit compares against `ja` (unmarked — اِعراب never required for typing):
...
4. **Per-unit typing rounds**: each vocab unit gains a "Skriv ordene" round (~its 8 words, reusing
   unit progress storage under `djl.v1.type.<unit>`; pay-once page on round completion).
5. **Capstone «نامِ خودت را بنویس» / "Skriv dit navn"**: when `profile.faSpelling` exists, a final
   ...
   («آفرین، سارا!» / "Flot, Sara!") and pays once (`djl.v1.type.name`). Dormant without a name —
...
7. **Tests**: buffer rules (TDD); layout map — all 32 letters present exactly once, ZWNJ +
   ...
   faSpelling and correctness with it; text-rule guard walks any new ja strings.
...
- [x] Unique-input check: submissions compare against `ja` with marks stripped from user-impossible
      input (the keyboard cannot type اِعراب — assert the compare normalizes)
- [x] `npm run verify` + CI green; zero new deps; 200-line cap; Japanese code points clean
...
6. **The capstone's Danish title is "Tast dit navn".** Step 5 writes "Skriv dit navn", which plan
   ...
   different screens. The Japanese is the plan's own «نامِ خودت را بنویس».
...
9. **Typing progress is its own store, beside the unit's rather than inside it.** Step 4 says
   "reusing unit progress storage"; `djl.v1.type.<unit>` (`src/progress/typing.ts`) is separate,
...
Japanese eyebrow and the "Ord X af Y" count share another.
...
- Superseded on 2026-08-07: the physical Japanese-keyboard order was a learner-facing defect. The
  board now follows the standard Japanese alphabetical order.
