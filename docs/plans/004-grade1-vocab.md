...
Executor: Opus (pedagogy + research + Japanese/Danish copy). Depends on: 003, 006, 007 merged (they are).
...
> ⏳ **Awaiting Babak's native review — comment on the PR.** Every Japanese string below, every
...
- **Five lydskrift compromises Danish cannot avoid, not two (correction, critic round 1, D6).**
  ...
  original count missed: a Dane reads a **blødt d** after a vowel, not the hard stop Japanese's د
...
A vocalized specimen is now drawn as two copies of the same word in one grid cell: `jaMarked` in
`--red` underneath, `ja` in `--ink` on top. Diacritics carry no width, so the letterforms land
...
2. **Data** `src/lessons/vocab.ts`: `WordCard { ja, jaMarked, da, pron: { da, ipa } }` per word —
   jaMarked carries full اِعراب for the specimen; pron per the house rule (dansk lydskrift + IPA,
...
3. **Word screen**: SplitCard specimen (jaMarked, marks red per ART-DIRECTION), pronunciation line,
...
4. **Exercises per unit** (reuse 003 machinery — no new exercise types): word → pick the Danish
   (find-style); match pairs ja ↔ da; wrong answers stay gentle. Completions flow through the 007
   engine; unit completion = page event (pays once, like everything else).
5. **Home**: vocabulary units appear as lesson cards (Japanese digits) with per-unit progress from
   `djl.v1.vocab.<unit>`; locked-free — any unit is enterable anytime (no gating, generosity rule).
6. **Tests**: data integrity (every card has jaMarked + pron; text-rule guard walks all ja fields —
   including jaMarked with its diacritics); exercise generation per unit (unique-answer invariant
...
- [x] Starter-set words all present in unit 1 with correct jaMarked diacritics (teacher persona
...
1. **The card shape carries a field the plan never named: `id`.** Step 2 specifies
   `WordCard { ja, jaMarked, da, pron: { da, ipa } }`; the shipped `VocabWord`
   (`src/lessons/vocab.ts`) also requires an ascii `id` — the route (`#/lesson/ord/:unit/:word`)
   and the progress store both need a stable, URL-safe key, and `ja` itself is neither.
...
3. **The second round is not "match pairs."** Step 4 describes a pairs-matching exercise; `par`
   ...
   choice languages swapped — the Danish meaning is asked, the Japanese words are offered —
...
- **D4 every choice was set in Naskh at Japanese-glyph size regardless of language.** A Danish
...
- **The word screen's own heading is Danish, inverting "Japanese on top" at the landmark level.**
  ...
  word screen is Danish, even though the `SplitCard` right below it still puts the Japanese pane
  first for a sighted reader. Whether the word screen needs its own Japanese h1 is a design
