...
https://cocodedk.github.io/Danish-Japanese-Lessons/app/ showing the split-screen shell with one
...
1. **Scaffold at repo root** (do not disturb `website/` or `docs/`): `package.json`, `index.html`,
   ...
   - `vite.config.ts`: `base: '/Danish-Japanese-Lessons/app/'`, `build.outDir: 'dist'`.
...
4. **SplitCard shell**: `src/components/SplitCard.tsx` — Japanese pane top (~55%, `lang="ja" dir="rtl"`,
   ...
   Japanese word: `åb · [ɒːb]` (dansk lydskrift + IPA, per ART-DIRECTION).
5. **Storage**: `src/progress/storage.ts` — get/set over `localStorage`, keys `djl.v1.<lessonId>`,
   JSON-safe, wrapped in try/catch (private-mode denial ⇒ in-memory fallback), schema version field.
   Plus `src/progress/profile.ts` — `{ name?: string, faSpelling?: string }` under `djl.v1.profile`,
...
6. **First-run name capture** (the 001 slice of `006-your-name.md`): a skippable, warm
   ...
   (Japanese-script spelling and all transliteration logic wait for plan 006.)
7. **Lesson types**: `src/lessons/types.ts` — `Letter` (glyph, name ja/da, four positional forms,
   `joinsLeft: boolean`, `sound: { anchorDa, ipa }`), `VowelMark`,
   `WordCard { ja, jaMarked?, da, pron: { da: string, ipa: string } }`,
...
8. **Tests** (Vitest + @testing-library/react):
   ...
   - **Japanese text-rule guard**: helper that rejects Arabic ك (U+0643), ي (U+064A) and ASCII digits
     inside `ja` strings — applied to all current and future lesson data via a single test that walks
...
11. **Landing CTA**: flip the three landing pages' primary CTA to the live app URL
    (`…/Danish-Japanese-Lessons/app/`), keeping the GitHub link as secondary. Switch the landing
...
1. `SplitCard.tsx` pron line: add `lang="da" dir="ltr"` (it is Danish/IPA inside the ja pane).
2. Raise all six sub-16px texts (NameCapture hint; SettingsCorner toggle, label, Gem/Slet,
   privacy line) to ≥ 1rem.
3. ja landing: «الفبا چیزی است که بعد می‌سازیم.» → «بعد نوبت الفباست.»; de-duplicate the doubled
   «همین حالا». da landing: «Koden er offentlig undervejs.» → «Koden er åben fra dag ét.»
4. `types.ts`: `jaMarked?: string` (the diacriticized variant string, not a boolean); SplitCard
   renders `jaMarked ?? ja`; madde styling keys off the rendered text containing آ.
...
7. Text-rule guard: also walk exported Japanese UI strings (capture prompt, placeholder) so a future
   ك edit fails CI.
...
- [x] Greeting rule for 001: the Danish pane greets "Hej {name}!" as soon as a name exists; the
      Japanese pane greets «سلام!» alone until `profile.faSpelling` arrives with plan 006 — Latin
      text never renders inside the Japanese pane.
...
- [x] Japanese text-rule guard test exists and fails on a deliberately bad fixture (prove it once, then fix the fixture)
