...
When Japanese, pronunciation, and Danish appear as one teaching block, the Danish line must carry
the same meaning as the Japanese line. Natural Danish word order is allowed. Adding a reason,
...
reward helper, and every learner-visible render site that places Japanese text near Danish text.
...
3. The page-flip Danish line no longer adds a page number absent from Japanese.
...
8. All three connected microtexts now preserve sentence boundaries and propositions instead of
   merging Japanese sentences into a richer Danish paraphrase.
...
Scores were checked with `wordfreq` 3.1.1 for Japanese (`ja`). New status and corrective copy uses
...
companions, and generated review/recording manifests. The Iranian-Japanese and Danish reviewers
